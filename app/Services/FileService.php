...
class FileService
{
    ...
    protected function renameAndMoveTo($file, $destination)
    {
        ...
        $fileName = $file->store($this->mainStorage . '/' . $destination);

        return [$fileOriginalName, $fileName, $file->getSize()];
    }

    public function moveTo($file, $destination)
    {
        $fileOriginalName = $file->getClientOriginalName() . $file->getClientOriginalName();
        $fileName = $file->store($this->mainStorage);
        return [$fileOriginalName, $fileName, $file->getSize()];
    }
    ...
    public function saveFile(Model $model, $file, $destination = '')
    {
        ...
        return $model->resources()->create([
            ...
            'fk_i_creator_id' => $authUser?->getKey(),
            's_filename' => $fileName,
            's_directory' => $destination,
            'i_size' => $uploadedFile[2] ?? 0
        ]);
    }
